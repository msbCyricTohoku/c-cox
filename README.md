...
Recently updated, C-Cox now fully supports advanced clinical trial and Electronic Health Record (EHR) methodologies, including **Time-Varying Covariates**, **Clustered Longitudinal Data**, and **Competing Risks**. The newly added features are extremely important in biostatistics.

---

## Source codes:

`ccox.c`  (old source - no time varying covariates)

`main.c`, `ccox_math.c`, `ccox.h`  (new source - time varying covariates, clustered longitudinal data, competing risks)
...
When patients have multiple clinic visits (multiple rows of data), their records are statistically correlated. A standard Cox model assumes every row is a completely independent person, which artificially shrinks error margins and leads to false discoveries. C-Cox solves this by applying the **Lin-Wei Robust Sandwich Estimator** grouped by patient clusters:
...
Standard survival curves (like Kaplan-Meier) fail when competing risks exist (example; a patient dies of an unrelated cause and can no longer experience the target event). C-Cox isolates users specific target event and calculates the true absolute probability over time using the **Aalen-Johansen Estimator**:
...
## Supported Dataset Style: Counting Process (Start and Stop Columns)

C-Cox relies on the counting process (Start and Stop) format. This seems to be standard for longitudinal data in modern biostatistics. It allows users to update a patient health data (like aging or fluctuating BMI) dynamically across multiple intervals by simply creating a new row per visit.
...
* **patient_id:** Groups rows belonging to the same subject (the clustering).
...
* **Covariates:** The age and BMI update dynamically at every visit.

---

## What C-Cox Offer?

* **Massive Parallelism:** Fully integrated OpenMP multithreading for rapid partial likelihood gradients across millions of rows.
* **Binary Search:** Custom binary search logic eliminates linear time bottlenecks in complex risk set calculations.
* **Minimal Dependencies:** Written in standard C, requiring only the GSL (GNU Scientific Library).
* **Tie Handling:** Mathematically exact Breslow method for tied survival times.
* **Memory Efficient:** Dynamically allocated arrays that tends to prevent stack overflow crashes on 10M+ row datasets.
* **Standard Cox:** If you leave advanced features out of the config, it acts as a standard, independent Right-Censored Cox model.
...
# --- Data Settings ---
file=dummy_medical_data.csv  # Path to your CSV dataset
...
# --- Column Mappers (Counting Process) ---
