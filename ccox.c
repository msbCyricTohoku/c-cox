// c-cox implementation of cox regression in c
// by msb
...
void ccox(DATA *dat, DATA_RES *res, int N, int COVNO, double **Z, int MAX_ITER,
  ...
  for (int iter = 0; iter < MAX_ITER; iter++) {
    ...
    gsl_matrix_memcpy(I_copy,
                      I); /// copy of the info matrix for beta and delta calc
  ...
  int s;
  //  gsl_matrix_view invH = gsl_matrix_view_array(&res->inv_hessian[0][0],
  //  covN, covN);
...
int main(int argc, char *argv[]) {
  ...
  if (fscanf(
          config,
          "MAX_ITER=%d\nTOLERANCE=%le\nfile=%s\nn=%d\ncovno=%d\ncovariates=%s",
          &MAX_ITER, &TOLERANCE, csv_file, &N, &COVNO, cov_line) != 6) {
  ...
  for (int j = 0; j < total_csv_cols; j++) {
    ...
    if (strcmp(header_cols[j], "week") == 0)
      time_idx = j;
    if (strcmp(header_cols[j], "arrest") == 0)
      status_idx = j;
  }
  ...
  while (fgets(line, sizeof(line), file) && row_count < N) {
    ...
    while (val_token != NULL) {
      ...
      if (current_col == time_idx)
        S1.time[row_count] = atof(val_token);
      if (current_col == status_idx)
        S1.status[row_count] = atoi(val_token);
  ...
  // printf("inv hessian %d  %f\n",k,result.inv_hessian[k]); //need diagonals
  // aka k=0, k=3 for 2x2 mat
  // }
  for (int k = 0; k < COVNO; k++) {
    ...
    double se =
        (variance > 0) ? sqrt(variance) : 0.0; // ensure + vals before sqrt
