...
int cmp_double(const void *a, const void *b) {
  ...
  return (da > db) - (da < db); /*------1 if da > db (true/false > 1 - 0), 0 if they equal and vice versa-----*/
}
/*-----------------------------------------------------------------------*/
...
#pragma omp for
  ...
  /*-----thread safe update, very important to avoid overwrite by different threads-----*/
#pragma omp critical
  {
...
/*---------------------newly added compute cif which is cumulative incidence function----------------*/
